...
test("release workflow validates the exact safe tag and committed registry", () => {
  ...
  assert.match(workflow, /test "\$tag_commit" = "\$head_commit"/u);
  assert.match(workflow, /test "\$tag_commit" = "\$GITHUB_SHA"/u);
});
...
test("release workflow uploads only builder-validated archive arguments", () => {
  ...
  assert.match(
    workflow,
    /gh release create "\$RELEASE_TAG" "\$\{archives\[@\]\}" --verify-tag --generate-notes/u,
  );

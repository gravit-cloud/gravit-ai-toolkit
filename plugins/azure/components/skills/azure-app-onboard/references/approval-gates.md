...
> ⛔ **After deploy approval:** Your NEXT action MUST be: read `deploy/SKILL.resource.md`, then read `.copilot-azure/sessions/{id}/deploy-checklist.md`. Do NOT call the `azure-deploy` skill — AppOnboard uses its own embedded deploy sub-skill.
...
> ⛔ **Before entering deploy:** ⛔ Read [`deploy/SKILL.resource.md`](../deploy/SKILL.resource.md) before any deployment action. After mid-session compaction, re-read `deploy/SKILL.resource.md` Steps 4-8. You MUST write `deploy-result.json`.
...
> ⛔ **Phase exit:** `deploy-result.json` MUST be written before proceeding to Step 9. See deploy/SKILL.resource.md for the full exit gate checklist.

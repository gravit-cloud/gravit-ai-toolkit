...
| **create** | Use when the standard end-to-end happy path doesn't fit — lifting existing agent code into the project, deploying outside the default code path, wiring connections at scaffold time, advanced setup, or recovering from a failed quickstart run. | create |
...
| Anything beyond the standard quickstart (existing code, deployment customization, scaffold-time connections, recovery) | create → [deploy](./foundry-agent/deploy/deploy.md) → [invoke](./foundry-agent/invoke/invoke.md) |

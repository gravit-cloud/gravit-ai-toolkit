...
Run all 4 dimensions before writing `prepare-plan.json`. All must pass — a failure in any dimension triggers the [Error Handling](../SKILL.resource.md#error-handling) procedures.
...
- **On failure:** Fix inline via the [Error Handling](../SKILL.resource.md#error-handling) procedures. Document tradeoffs (e.g., WAF Reliability vs cost-optimized budget) in `assumptions[]`.
